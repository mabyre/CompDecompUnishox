...
#define USX_FREQ_SEQ_TXT \
  (const char *[]) { " the ", " and ", "tion", " with", "ing", "ment" }
#define USX_FREQ_SEQ_URL \
  (const char *[]) { "https://", "www.", ".com", "http://", ".org", ".net" }
#define USX_FREQ_SEQ_JSON \
  (const char *[]) { "\": \"", "\": ", "\",", "}}}", "\":\"", "}}" }
#define USX_FREQ_SEQ_HTML \
  (const char *[]) { "</", "=\"", "div", "href", "class", "<p>" }
#define USX_FREQ_SEQ_XML \
  (const char *[]) { "</", "=\"", "\">", "<?xml version=\"1.0\"", "xmlns:", "://" }

#define USX_TEMPLATES \
  (const char *[]) { "tfff-of-tfTtf:rf:rf.fffZ", "tfff-of-tf", "(fff) fff-ffff", "tf:rf:rf", 0 }
